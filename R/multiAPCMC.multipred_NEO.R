...
#'                      cuttrend=rep(0,15))
#'
multiAPCMC.multipred_NEO <- function(multiAPCMC.multifit.object,futuredata,cuttrend) {
    ...
    for(n in 1:length(vec.noperiod)) {
      ...
      for(p in 1:length(vec.refper)) {
        ...
        for(c in 1:length(vec.refcoh)) {
          ...
          np.pred[[l]][[n]][[p]][[c]] <-  multiAPCMC.singlepred_NEO(multiAPCMC.multifit.object$np.est[[l]][[n]][[p]][[c]],
                                                           futuredata,
                                                           noproj,
                                                           cuttrend)
