...
Note that the curve we see here is rather treacherous: most trends will not look like this empirically speaking. So I think we model does decently.
...
If we have a 'training set' and a 'validation set', what do we then do when we have decided what the best model parameters are? Do we then use the full set? One idea is a combination of (1) and (2). Namely, we could use the full dataset from (2), but we use the cohort and period references from (1). If we want to do this, we cannot use multiAPCMC.retrievemodel() because it refers to cohorts and periods by "extremes" or "middle". However, what is "extreme" or what is "middle" will change when we add years to the dataset. Instead, we can use multiAPCMC.datashape() and multiAPCMC.singlefit. These functions allow us to refer to the exact cohort and period references. I show this below:
